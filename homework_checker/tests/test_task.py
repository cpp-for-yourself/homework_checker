...
class TestTask(unittest.TestCase):
    ...
    def test_injecting_new(self):
        ...
        checker = Checker(
            "homework_checker/tests/data/dummy/dummy_solution/solution.yml"
        )
        ...
        self.assertFalse(path.isdir(path.join(task._backup_folder, folder_to_inject)))
        self.assertFalse(
            path.exists(path.join(task._backup_folder, folder_to_inject, "blah.cpp"))
        )
    ...
    def test_injecting_existing(self):
        ...
        self.assertTrue(
            path.exists(
                path.join(task._student_task_folder, folder_to_inject, "CMakeLists.txt")
            )
        )
        self.assertTrue(
            path.exists(
                path.join(task._student_task_folder, folder_to_inject, "test_dummy.cpp")
            )
        )
        task._revert_injections(folder_to_inject)
        self.assertFalse(path.isdir(path.join(task._backup_folder, folder_to_inject)))
        self.assertFalse(
            path.exists(
                path.join(task._backup_folder, folder_to_inject, "CMakeLists.txt")
            )
        )
        self.assertFalse(
            path.exists(
                path.join(task._backup_folder, folder_to_inject, "test_dummy.cpp")
            )
        )
